...
NOM_ENTREPRISE = "VERSIGENT"
LARGEUR_LIGNE = 32  # largeur utile en caractères (police normale, imprimante 80mm)
...
def ligne_justifiee(gauche: str, droite: str, largeur: int = LARGEUR_LIGNE) -> str:
    """Renvoie une ligne de texte avec 'gauche' à gauche et 'droite' aligné à droite."""
    espace = largeur - len(gauche) - len(droite)
    if espace < 1:
        espace = 1
...
class Imprimante:
    ...
    def imprimer_ticket(self, numero_badge: str, numero_ticket: int,
        ...
        identite = f"{prenom_affiche} {nom_affiche}".strip()
        ...
        try:
            ...
            data += TAILLE_DOUBLE
            data += BOLD_ON
            data += enc(NOM_ENTREPRISE) + SAUT_LIGNE
            data += BOLD_OFF
            data += TAILLE_NORMALE
            ...
            # -- Heure (gauche) / Date (droite) sur une même ligne --
            data += ALIGN_LEFT
            data += enc(ligne_justifiee(f"Heure : {heure_str}", f"Date : {date_str}")) + SAUT_LIGNE
            data += ALIGN_CENTER
            data += SAUT_LIGNE

            # -- Prénom + Nom (une seule ligne) --
            data += BOLD_ON
            data += enc(identite) + SAUT_LIGNE
            data += BOLD_OFF
            data += SAUT_LIGNE
